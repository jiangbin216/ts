        ...
        <div class="form-group">
			<span class="type1 type2 type3">
				<select id="systemId" name="systemId">
					...
					<c:forEach var="upmsSystem" list="${upmsSystems}">
                        <option value="${upmsSystem.systemId}"
                                <c:if test="${permission.systemId==upmsSystem.systemId}">selected="selected"</c:if>>${upmsSystem.title}</option>
                    </c:forEach>
				</select>
			</span>
        ...
        <div class="radio">
            <div class="radio radio-inline radio-success">
                <input id="status_1" type="radio" name="status" value="1"
                       <c:if test="${permission.status==1}">checked</c:if>>
                <label for="status_1">正常 </label>
            </div>
            <div class="radio radio-inline">
                <input id="status_0" type="radio" name="status" value="0"
                       <c:if test="${permission.status==0}">checked</c:if>>
                <label for="status_0">锁定 </label>
            </div>
        </div>
        <div class="form-group text-right dialog-buttons">
            <a class="waves-effect waves-button" href="javascript:void(0);" onclick="updateSubmit();">保存</a>
...
<script type="text/javascript">
    ...
    function initPid(val) {
        var $pid = $('#pid');
        if (systemId != 0) {
            $.getJSON('${ctx}/manage/permission/list', {
                systemId: systemId,
                type: pidType,
                limit: 10000
            }, function (json) {
    ...
    function updateSubmit() {
        $.ajax({
            type: 'post',
            url: '${ctx}/manage/permission/update/${permission.permissionId}',
