...
$(function () {
    ...
    $('.switch-systems').click(function () {
        ...
        $.cookie('ts-upms-systemid', systemid);
        $.cookie('ts-upms-systemname', systemname);
        $.cookie('ts-upms-systemtitle', systemtitle);
    });
    // 显示cookie菜单
    var systemid = $.cookie('ts-upms-systemid') || 1;
    var systemname = $.cookie('ts-upms-systemname') || 'ts-upms-web';
    var systemtitle = $.cookie('ts-upms-systemtitle') || '权限管理系统';
    $('.system_menus').hide(0, function () {
        $('.system_' + systemid).show();
    });
    $('body').attr('id', systemname);
