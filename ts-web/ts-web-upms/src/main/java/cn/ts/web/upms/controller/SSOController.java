...
import cn.ts.web.upms.shiro.session.UpmsSimpleSession;
 ...
 * 单点登录管理
 *
 * @author Created by YL on 2016/12/10.
 */
@Controller
...
public class SSOController extends BaseController {
    ...
    // 全局会话key
    private final static String TS_UPMS_WEB_SESSION_ID = "ts-upms-web-session-id";
    // 全局会话key列表
    private final static String TS_UPMS_WEB_SESSION_IDS = "ts-upms-web-session-ids";
    // code key
    private final static String TS_UPMS_WEB_CODE = "ts-upms-web-code";
    ...
    public String index(HttpServletRequest request) throws Exception {
        ...
        int count = upmsSystemService.selectCountByExample(upmsSystemExample);
        if (0 == count) {
    ...
    public String login(HttpServletRequest request) {
        ...
        // 判断是否已登录，如果已登录，则回跳
        String code = JedisUtil.get(redisTemplate, TS_UPMS_WEB_SESSION_ID + "_" + serverSessionId);
        // code校验值
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        String hasCode = JedisUtil.get(redisTemplate, TS_UPMS_WEB_SESSION_ID + "_" + sessionId);
        // code校验值
        if (StringUtils.isBlank(hasCode)) {
            ...
            // 更新session状态
            upmsCachingSessionDAO.updateStatus(sessionId, UpmsSimpleSession.OnlineStatus.on_line);
            // 全局会话sessionId列表，供会话管理
            JedisUtil.lpush(redisTemplate, TS_UPMS_WEB_SESSION_IDS, sessionId.toString());
            // 默认验证帐号密码正确，创建code
            String code = UUID.randomUUID().toString();
            // 全局会话的code
            JedisUtil.set(redisTemplate, TS_UPMS_WEB_SESSION_ID + "_" + sessionId, code, (int) subject.getSession
                    ().getTimeout()
                    / 1000, TimeUnit.SECONDS);
            // code校验值
            JedisUtil.set(redisTemplate, TS_UPMS_WEB_CODE + "_" + code, code, (int) subject.getSession()
                    .getTimeout() / 1000, TimeUnit.SECONDS);
        }
        // 回跳登录前地址
        String backurl = request.getParameter("backurl");
        if (StringUtils.isBlank(backurl)) {
    ...
    public Object code(HttpServletRequest request) {
        ...
        String code = JedisUtil.get(redisTemplate, TS_UPMS_WEB_CODE + "_" + codeParam);
